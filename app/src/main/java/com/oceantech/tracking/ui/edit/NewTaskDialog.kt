...
class NewTaskDialog(
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        ...
        binding.confirmNewTask.setOnClickListener{
            val oh = with(binding.etNewOH.text.toString()) {if(this.isEmpty()) 0.0 else this.toDouble()}
            val ot = with(binding.etNewOT.text.toString()) {if(this.isEmpty()) 0.0 else this.toDouble()}
    ...
    private fun listenToChanges() {
        binding.etNewOH.addTextChangedListener(object : TextWatcher{
            ...
            override fun onTextChanged(s: CharSequence?, start: Int, before: Int, count: Int) {
                ...
                binding.tvNewOHTask.text = if(!s.isNullOrEmpty()) getString(R.string.office_task_star) else getString(R.string.office_task)
            }
        })
        binding.etNewOT.addTextChangedListener(object : TextWatcher{
            ...
            override fun onTextChanged(s: CharSequence?, start: Int, before: Int, count: Int) {
                ...
                binding.tvNewOTTask.text = if(!s.isNullOrEmpty()) getString(R.string.overtime_task_star) else getString(R.string.overtime_task)
            }
        })
